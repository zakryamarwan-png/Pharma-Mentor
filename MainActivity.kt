...
class MainActivity : ComponentActivity() {
    ...
    private fun showDrugMasterProfile(drugId:String, back:String){ lifecycleScope.launch {
        ...
        val allDrugsForBack = repo.allDrugs()
        l.addView(button("رجوع"){if(back=="results") showDrugResults(allDrugsForBack) else drugLibrary()})
        l.addView(button("الرئيسية"){home()})
        setScreen(l)
    } }
    ...
    private fun interactionsForDrug(name:String){ lifecycleScope.launch {
        ...
        val allDrugsForBack = repo.allDrugs()
        val backDrugId = allDrugsForBack.firstOrNull { it.name.equals(name,true) || it.name.contains(name,true) }?.id ?: ""
        l.addView(button("رجوع"){showDrugMasterProfile(backDrugId, "library")})
        setScreen(l)
    } }
